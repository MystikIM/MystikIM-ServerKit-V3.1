...
### Fixed
- `SpawnPosition.ClrIdx` removed from the game API — zeroed out where read.
- `GameManager.SavePersistentPlayerData()` → moved to `persistentPlayers`/`GetPersistentPlayerList()`.
- `Prefab.SleeperVolumes.Clear()` → replaced with a loop calling `.Reset()` per entry (the type no longer supports bulk clear).
- `Bag` → `ItemStack[]` via `Bag.GetSlots()`.
- `Localization.dictionary` → `Localization.Dictionary`.
- `DynamicPrefabDecorator.GetDynamicPrefabs()`/`GetPOIPrefabs()` → now fields (`allPrefabs`/`poiPrefabs`) instead of methods.
- `BlockLandClaim` type removed entirely — land claim blocks are now generic `CompositeTileEntity` blocks tagged `"landclaim"`; updated all type/placement checks accordingly.
- `BlockChangeInfo.pos`/`.clrIdx` → `.blockValueRef.BlockPosition`; constructor now takes a `BlockValueRef`.
- `World.ChunkClusters[0]` collapsed into a single `World.ChunkCache` field.
- `GameManager.CollectEntityServer(entityId, collectorId)` removed — replaced by an instance method, `entity.Collect(collectorEntityId)`.
- `Prefab.PrefabTeleportVolume` moved to `PrefabVolumes.PrefabTeleportVolume`; its list type now requires a backing `Prefab` instance in its constructor.
- Zombie-kill and death-penalty chat notifications were hardcoded Chinese strings with no dashboard setting to change them — translated to English.
- Build: pinned the `System.Runtime.InteropServices.RuntimeInformation.dll` copy step to a fixed path so `dotnet build` succeeds without Visual Studio installed.
