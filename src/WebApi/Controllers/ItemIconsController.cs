namespace SdtdServerKit.WebApi.Controllers
{
    ...
    public class ItemIconsController : ApiController
    {
        ...
        private IHttpActionResult InternalGet(string iconFileName, int r, int g, int b)
        {
            ...
            try
            {
                byte[] resultData = IconTinter.TintPng(iconPath, r, g, b, skipFullyTransparent: false);
                return new FileContentResult(resultData, "image/png");
            }
            catch (Exception ex)
            {
                Log.Error($"[ItemIcons] 染色失败 ({iconFileName}, RGB={r:X2}{g:X2}{b:X2}): {ex}");
        ...
        private IHttpActionResult InternalGet(string iconFileName, UnityEngine.Color color)
        {
            ...
            try
            {
                byte[] resultData = IconTinter.TintPng(iconPath, r, g, b, skipFullyTransparent: true);
                return new FileContentResult(resultData, "image/png");
            }
            catch (Exception ex)
            {
