    ...
    public class ModApi : IModApi
    {
        ...
        private static void LoadAppSettings()
        {
            try
            {
                ...
                ProductionAppConfigPath = productionAppConfigPath;
                ...
                CustomLogger.Configure(AppSettings.LogLevel, AppSettings.EnableConsoleOutput);
            }
            catch (Exception ex)
            {
        ...
        public static string ProductionAppConfigPath { get; private set; } = null!;
        /// <summary>
        /// 是否需要进行首次初始化（尚未设置过管理员账号密码）。
        /// </summary>
        public static bool NeedInitialize =>
            AppSettings != null
            && (string.IsNullOrEmpty(AppSettings.UserName) || string.IsNullOrEmpty(AppSettings.Password));
        public static void SaveAppSettings()
        {
            string path = ProductionAppConfigPath ?? Path.Combine(AppContext.BaseDirectory, "LSTY_Data", "appsettings.json");
        ...
        private static void PatchByHarmony()
        {
            try
            {
                ...
                CustomLogger.Debug("Patch all by harmony success.");
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        private static void OnCalcChunkColorsDone(ref ModEvents.SCalcChunkColorsDoneData _data)
        {
            Functions.MapTileRenderer.OnChunkColorsDone(_data.Chunk);
        }
        private static void RegisterModEventHandlers()
        {
            try
            {
                ...
                ModEvents.GameMessage.RegisterHandler(ModEventHub.OnGameMessage);

                ModEventHub.GameStartDone += OnGameStartDone;

                CustomLogger.Debug("Registered mod event handlers success.");
            }
            catch (Exception ex)
            {
        ...
        private static void OnGameStartDone()
        {
            try
            {
                ...
                try
                {
                    // 接管原 TFP_MapRendering.dll 的实时地图更新职责：
                    // 监听 chunk 颜色计算完成事件，由 MapTileRenderer 增量刷新瓦片。
                    // 不再依赖 TFP_MapRendering.dll（地图渲染、读图、实时更新均由本项目自实现）。
                    string saveGameDir = GameIO.GetSaveGameDir();
                    ModEvents.CalcChunkColorsDone.RegisterHandler(
                        new ModEvents.ModEventHandlerDelegate<ModEvents.SCalcChunkColorsDoneData>(OnCalcChunkColorsDone));
                    Functions.MapTileRenderer.EnableIncremental(saveGameDir);
                }
                catch (Exception ex)
                {
                ...
                CustomLogger.Info("游戏世界加载完成，所有服务已启动。");
            }
            catch (Exception ex)
            {
        ...
        private static void InitDatabase()
        {
            try
            {
                ...
                TryAddColumn(dbConnection, "T_LotteryRecord_v1", "PlayerName", "TEXT");
                TryAddColumn(dbConnection, "T_LotteryRecord_v1", "CreatedAt", "TEXT");
                TryAddColumn(dbConnection, "T_CityLocation_v1", "IsEnabled", "INTEGER NOT NULL DEFAULT 1");
                TryAddColumn(dbConnection, "T_CityLocation_v1", "TeleCommand", "TEXT");
                TryAddColumn(dbConnection, "T_ItemList", "DisplayName", "TEXT");
                TryAddColumn(dbConnection, "T_ItemList", "RewardType", "INTEGER NOT NULL DEFAULT 0");
                TryAddColumn(dbConnection, "T_CommandList", "DisplayName", "TEXT");

                CustomLogger.Debug("Initialize database success.");
            }
            catch (Exception ex)
            {
